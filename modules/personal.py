...
def press_select():
    ...
    if actMod==True:
        if movimientos[actPos][2] == u"peso":
            ...
            base_de_datos.actualizar_insudesayuno(datos[actPos])
        elif movimientos[actPos][2] == u"rata":
            base_de_datos.actualizar_insualmuerzo(datos[actPos])
        else:
            base_de_datos.actualizar_insucena(datos[actPos])
        #print datos[actPos]
        actMod=False
    else:        
        actMod=True
    ...
    if actMod==True:
        ...
        if desp>0:
            ...
            if actPos == 0 or actPos == 1:
                ...
                if datos[actPos]<1:
                    datos[actPos]=1
            elif actPos == 3 or actPos == 4 or actPos == 5:
                if datos[actPos]<0:
                    datos[actPos]=0
            else:
...
def press_right():
    moverCursor(0.01,1,1,1)

def press_down():
    moverCursor(-5,-50,-5,2)

def press_left():
    moverCursor(-0.01,-1,-1,3)
...
def mostrar_personal(vAtras):
    ...
    for i in range(6):
        ...
        #ratioCena = 2
            ...
            datos[i] = base_de_datos.obtener_insudesayuno_actual()
        elif i == 4:
            datos[i] = base_de_datos.obtener_insualmuerzo_actual()
        else:
            datos[i] = base_de_datos.obtener_insucena_actual()
    global actPos
    actPos=0
    global actMod
    actMod=False
